...
以下为 v0.5.0 在 macOS 上的真实运行截图，窗口均为 980 × 720，使用同一组 18 个虚构应用、分组、置顶和演示历史。
所有应用名称、图标、备注、路径与使用记录均为演示数据，不使用个人应用名单。

| 极简模式 · 默认 | 常规模式（应用内称“标准模式”） |
| --- | --- |
| ![使用虚构演示应用的极简模式](docs/images/minimal-main-zh-CN.jpg) | ![使用相同虚构演示应用的常规模式](docs/images/standard-main-zh-CN.jpg) |
| 单一应用网格、搜索和紧凑筛选，详情按需打开。 | 保留置顶区、分组侧栏、详细列表、最近使用和常驻详情。 |
| [查看原尺寸截图](docs/images/minimal-main-zh-CN.jpg) | [查看原尺寸截图](docs/images/standard-main-zh-CN.jpg) |

两种模式共享应用数据和管理能力。在极简模式通过 **更多 → 设置 → 界面模式** 切换，
常规模式通过 **设置 → 界面模式** 切换。已有明确选择会保留；没有保存偏好的用户默认进入极简模式。
...
正式签名及公证仍是独立的后续工作。v0.3.0 及更早版本需手动安装首次支持更新的版本；后续更新需正式版发布及用户确认。
...
![Hatch 在 macOS 上的界面模式与语言设置](docs/images/minimal-settings-zh-CN.jpg)
